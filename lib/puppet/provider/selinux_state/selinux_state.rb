Puppet::Type.type(:selinux_state).provide(:selinux_state) do
  ...
  def relabel?(should)
    ...
    on_cases.include?(should) && off_cases.include?(self.ensure)
  end

  def ensure
    return 'disabled' if String(Facter.value(:selinux)) == 'false'
    Facter.value(:selinux_current_mode).downcase
  end
  ...
  def ensure=(should)
    ...
    return unless resource[:autorelabel] && relabel?(String(should))
    touch '/.autorelabel'
  end
end
